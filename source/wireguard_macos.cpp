...
namespace wg
{
	...
	Result<std::string, int> macos_get_real_interface(const std::string& wg_iface, bool quiet)
	{
		...
		if(not stdfs::exists(name_path))
			return quiet ? Err(0) : msg::error("Interface '{}' does not exist", wg_iface);
		...
		if(stat(name_path.c_str(), &st) < 0)
			return quiet ? Err(0) : msg::error("stat('{}'): {} ({})", name_path, strerror(errno), errno);

		auto name_mtime = st.st_mtime;
		if(stat(socket_path.c_str(), &st) < 0)
			return quiet ? Err(0) : msg::error("stat('{}'): {} ({})", socket_path, strerror(errno), errno);

		auto socket_mtime = st.st_mtime;
		if(auto diff = socket_mtime - name_mtime; diff < -2 || diff > 2)
			return quiet ? Err(0) : msg::error("Interface name/socket out of sync!");
